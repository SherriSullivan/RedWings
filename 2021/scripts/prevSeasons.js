...
function createTable(year) {
    ...
    // loop through all the teams in prevSeasons.js
    // teams start in row 3
    for (let i = 2, l = prevSeasons.length; i < l; i++) {
        // create a row for each team
        tr = document.createElement("tr");
        // loop through all the years and prevSeasons for that team
        for (let j = 0, t = prevSeasons[i].length; j < t; j++) {
            // j1 is team abbreviation - skip j1
            if (j !== 1) {
                // add team and prevSeasons as data cells
                td = document.createElement("td");
                td.innerText = prevSeasons[i][j];
            	tr.appendChild(td);
            }

            // add team abbr as id to team row
            tr.setAttribute("id", prevSeasons[i][1]);

            // display only selected year
            if ((j > 1) && (prevSeasons[0][j] != year)) { td.style.display = "none"; }

            // if teamID matches stanleyID add class 'stanley'
            if ((prevSeasons[0][j] == year) && (prevSeasons[i][1] == prevSeasons[1][j])) {
                tr.setAttribute("class", "stanley");
            }

            // if there is no team data for that year, don't display the row
            if ((prevSeasons[0][j] == year) && (prevSeasons[i][j] === '') || (prevSeasons[i][j] === null)) {
